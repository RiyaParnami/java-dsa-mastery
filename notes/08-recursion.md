...
### Matching a familiar recurrence to this form

For a simple recurrence like:
```
T(N) = T(N/2) + C
```
matching against the general form:
```
a₁ = 1
b₁ = 1/2
ε₁(n) = 0
...
### Reading a recurrence: "what you get back" + "what you do here"

For a recurrence like:
```
T(N) = 2·T(N/2) + (N-1)
```
- `2·T(N/2)` → the answer you get back from the two recursive sub-calls (each on half the input)
- `(N-1)` → the extra work you are doing at **this** level (e.g. merging/combining results), which takes `(N-1)` time

**General rule:** `T(N) = (what you get from the recursive calls) + (what you are doing yourself, and how much time that takes)`.

### Example — deriving a recurrence from splitting an array in half

If a problem of size `N` is split into two halves of size `N/2` each, and merging them back together takes `(N-1)` time:
```
T(N) = T(N/2) + T(N/2) + (N-1)
